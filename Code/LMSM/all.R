...
mRNA_expr=t(gene_expr[geneProbes$external_gene_id[geneProbes$gene_biotype=="protein_coding"],])
lncRNA_expr=t(gene_expr[geneProbes$external_gene_id[geneProbes$gene_biotype=="lncRNA"],])


miRNA_Exp_SummarizedExperiment=t(mir_expr)
...
lncRNA_Exp_SummarizedExperiment=mRNA_expr
...
mRNA_Exp_SummarizedExperiment=lncRNA_expr
...
CandidateModulegenes_WGCNA <- module_WGCNA(lncRNA_expr, mRNA_expr
                                           , RsquaredCut = 0.8)
CommonmiRs_WGCNA <- share_miRs(t(mir_expr), lncRNA_expr, mRNA_expr, miRTarget, CandidateModulegenes_WGCNA)

LMSM_WGCNA <- LMSM(t(mir_expr), lncRNA_expr, mRNA_expr, miRTarget, CandidateModulegenes_WGCNA)
...
expr <- t(cbind(lncRNA_expr, mRNA_expr))
gsva_es <- gsva(expr, LMSM_WGCNA_Modulegenes, mx.diff = FALSE)
rownames(gsva_es) <- paste("LMSM", seq_along(LMSM_WGCNA_Modulegenes), sep=" ")
...
save.image("LMSM/LMSM_scz_bp_WGCNA_0.8_miRNA_lncRNA_mRNA.RData")
write.table(LMSM_WGCNA_Modulegenes[[13]],"LMSM/LMSM_13.txt",row.names = F,col.names = F,quote = F)
